...
static bool host_memory_backend_get_thp(Object *obj, Error **errp)
{
    return MEMORY_BACKEND(obj)->thp;
}

static void host_memory_backend_set_thp(Object *obj, bool value, Error **errp)
{
    HostMemoryBackend *backend = MEMORY_BACKEND(obj);

    if (host_memory_backend_mr_inited(backend)) {
        error_setg(errp, "cannot change THP policy after memory allocation");
        return;
    }
    backend->thp = value;
}
...
host_memory_backend_memory_complete(UserCreatable *uc, Error **errp)
{
    ...
    if (bc->alloc) {
        ...
        /* Set before preallocation, firmware/device writes and VFIO pinning.
         * Changing an existing THP VMA's policy would not split its folios. */
        if (!backend->thp &&
            qemu_madvise(ptr, sz, QEMU_MADV_NOHUGEPAGE)) {
            error_setg_errno(&local_err, errno, "cannot disable RAM THP");
            goto out;
        }
...
host_memory_backend_class_init(ObjectClass *oc, void *data)
{
    ...
    object_class_property_add_bool(oc, "thp", host_memory_backend_get_thp,
                                   host_memory_backend_set_thp);
    object_class_property_set_description(oc, "thp",
        "Allow transparent huge pages (set before any backing allocation)");
