...
int kvm_cpu_exec(CPUState *cpu)
{
    ...
    do {
        ...
        if (run_ret < 0) {
            ...
            if ((run_ret == -EFAULT || run_ret == -EHWPOISON) &&
                run->exit_reason == KVM_EXIT_MEMORY_FAULT &&
                (run->memory_fault.flags & KVM_MEMORY_EXIT_FLAG_CHAMELEON)) {
                error_report("Chameleon retained GPA fault: flags=0x%" PRIx64
                             " gpa=0x%" PRIx64 " size=0x%" PRIx64,
                             (uint64_t)run->memory_fault.flags,
                             (uint64_t)run->memory_fault.gpa,
                             (uint64_t)run->memory_fault.size);
                ret = -1;
                break;
            }
