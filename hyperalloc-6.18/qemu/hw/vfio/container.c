                                  ...
                                  Error **errp)
{
    ...
    /* Chameleon coordinates exact GPA unpinning through its RAM discard
     * listener. A guest vIOMMU would bypass that listener and leave a second
     * set of DMA mappings outside the reclaim transaction. */
    Object *balloon = object_resolve_path_type("",
                         "virtio-llfree-balloon-device", NULL);
    if (balloon && object_property_get_bool(balloon, "chameleon", NULL) &&
        (!object_property_get_bool(balloon, "chameleon-vfio", NULL) ||
         as != &address_space_memory)) {
        error_setg(errp, "Chameleon VFIO requires chameleon-vfio=on and direct GPA mappings without a guest IOMMU");
        return -EINVAL;
    }
