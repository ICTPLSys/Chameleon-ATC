		...
		struct mm_walk *walk)
{
	...
	if (pte_present(ptent)) {
		...
		if (!non_swap_entry(swpent)) {
			...
			/* Shadow retains resident mapping refs/rmap/RSS. Treat
			 * it as resident for PSS/private accounting as well. */
			if (is_device_private_entry(swpent) ||
			    is_chameleon_shadow_entry(swpent))
				present = true;
				...
				size_t count, loff_t *ppos)
{
	...
	if (mm) {
		...
		/* Restore before touching PTE access/soft-dirty state. In
		 * particular, clearing soft-dirty must also write-protect the
		 * restored present PTE so the next write is observed. */
		rv = chameleon_shadow_restore_range(mm, 0, ULONG_MAX);
		if (rv) {
			count = rv;
			goto out_unlock;
		}
