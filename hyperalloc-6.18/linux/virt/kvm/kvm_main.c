				 ...
				 const struct kvm_userspace_memory_region2 *mem)
{
	...
	if (!mem->memory_size) {
		...
		/* Deletion returns before the create/update gate below. */
		r = kvm_chameleon_memslot_change(kvm, old, NULL);
		if (r)
			return r;
#endif
...
#if IS_ENABLED(CONFIG_KVM_X86)
	/* Reject before kvm_set_memslot() invalidates the old slot and zaps
	 * translations; even a rejected mutation must preserve the transaction. */
	r = kvm_chameleon_memslot_change(kvm, old, new);
	if (r)
		goto out_unbind;
#endif
