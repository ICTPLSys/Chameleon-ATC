...
intel_pmu_save_and_restart_reload(struct perf_event *event, int count)
{
	...
	if (unlikely((x86_pmu.flags & PMU_FL_HYPERALLOC_PEBS_MEMINFO) &&
		     (event->attach_state & PERF_ATTACH_PEBS_CALLBACK) &&
		     (new > 0 || new < -(s64)period))) {
		/* The private KVM counter can escape the auto-reload interval
		 * after a virtual PMI/reprogram transition. Preserving -new as
		 * period_left then permits a near-2^47 period and no more PEBS.
		 * Repair only this opted-in consumer, while the PMU is disabled.
		 * This also runs on the native task-switch drain when no record
		 * arrived, so recovery does not depend on another working PMI.
		 *
		 * The invalid virtual raw delta is not an accurate event count.
		 * Account only complete records here: after a repair event->count
		 * is approximate. Chameleon heat uses the actual record callbacks
		 * and its PTW measurements use separate events. */
		local64_add(count * period, &event->count);
		local64_set(&hwc->period_left, period);
		static_call(x86_pmu_set_period)(event);
		chameleon_pebs_reloaded(new_raw_count);
		return 0;
	}
...
/* A bulk drain represents several samples but at most one PMI. Only kernel
 * consumers explicitly opting in receive each record through their callback,
 * without accounting each record as an interrupt. Other events are unchanged. */
static __always_inline void
intel_pmu_pebs_output(struct perf_event *event, struct perf_sample_data *data,
		     struct pt_regs *regs)
{
	if (event->attach_state & PERF_ATTACH_PEBS_CALLBACK)
		       ...
		       setup_fn setup_sample)
{
	...
	intel_pmu_pebs_output(event, data, regs);
}
