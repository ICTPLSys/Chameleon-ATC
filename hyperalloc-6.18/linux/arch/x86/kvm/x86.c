...
int kvm_vm_ioctl_check_extension(struct kvm *kvm, long ext)
{
	...
	case KVM_CAP_HYPERALLOC_PEBS_MEMINFO:
		...
		break;
	case KVM_CAP_CHAMELEON_RECLAIM:
		r = kvm_chameleon_supported();
		break;
			    ...
			    struct kvm_enable_cap *cap)
{
	...
	switch (cap->cap) {
	case KVM_CAP_CHAMELEON_RECLAIM:
		return kvm_chameleon_enable(kvm, cap);
	case KVM_CAP_HYPERALLOC_PEBS_MEMINFO:
		r = -EINVAL;
...
int kvm_arch_vm_ioctl(struct file *filp, unsigned int ioctl, unsigned long arg)
{
	...
	switch (ioctl) {
	case KVM_CHAMELEON_BEGIN:
	case KVM_CHAMELEON_REPORT:
	case KVM_CHAMELEON_INSTALL:
	case KVM_CHAMELEON_QUERY:
	case KVM_CHAMELEON_STATS:
	case KVM_CHAMELEON_TUNING:
		return kvm_chameleon_ioctl(kvm, ioctl, arg);
