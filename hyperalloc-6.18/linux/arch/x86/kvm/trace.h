...
TRACE_EVENT(kvm_chameleon,
	TP_PROTO(u64 transaction, u32 phase, u32 ranges, u64 pages,
		 u64 nativeflush_seq),
	TP_ARGS(transaction, phase, ranges, pages, nativeflush_seq),
	TP_STRUCT__entry(
		__field(u64, transaction)
		__field(u32, phase)
		__field(u32, ranges)
		__field(u64, pages)
		__field(u64, nativeflush_seq)
	),
	TP_fast_assign(
		__entry->transaction = transaction;
		__entry->phase = phase;
		__entry->ranges = ranges;
		__entry->pages = pages;
		__entry->nativeflush_seq = nativeflush_seq;
	),
	TP_printk("transaction=%llu phase=%s ranges=%u pages=%llu nativeflush_seq=%llu",
		__entry->transaction,
		__print_symbolic(__entry->phase, {1, "BEGIN_ZAP"},
			{2, "FLUSH_DONE"}, {3, "REPORT"}, {4, "INSTALL"}),
		__entry->ranges, __entry->pages, __entry->nativeflush_seq)
);
