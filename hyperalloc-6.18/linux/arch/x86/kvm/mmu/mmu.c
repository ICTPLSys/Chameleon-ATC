			      ...
			      const struct kvm_memory_slot *slot, gfn_t gfn)
{
	...
	return kvm_chameleon_max_level(kvm, slot, gfn,
				       min(host_level, max_level));
}
				 ...
				 struct kvm_page_fault *fault)
{
	...
	/* Chameleon serializes all fault-in with BEGIN. An asynchronous worker
	 * would perform GUP outside that gate, possibly after backing discard. */
	if (!kvm_chameleon_enabled(vcpu->kvm) &&
...
static int kvm_mmu_faultin_pfn_guarded(struct kvm_vcpu *vcpu,
			       struct kvm_page_fault *fault, unsigned int access)
{
	...
	ret = kvm_chameleon_fault(vcpu, fault);
...
static int kvm_mmu_faultin_pfn(struct kvm_vcpu *vcpu,
			     struct kvm_page_fault *fault, unsigned int access)
{
	struct rw_semaphore *gate = kvm_chameleon_fault_gate(vcpu->kvm);
	int ret;

	if (IS_ERR(gate))
		return fault->prefetch ? -EAGAIN : RET_PF_RETRY;
	ret = kvm_mmu_faultin_pfn_guarded(vcpu, fault, access);
	if (gate)
		up_read(gate);
	return ret;
}
				...
				struct kvm_page_fault *fault)
{
	 ...
	 */
	return fault->slot &&
	       (kvm_chameleon_fault_blocked(vcpu->kvm, fault->slot, fault->gfn) ||
