...
#ifdef CONFIG_LLFREE
	if (zone->llfree) {
		/* Same conservative availability estimate as the artifact. */
		...
		if (suitable_order >= HPAGE_PMD_ORDER)
			info->free_blocks_suitable = llfree_free_huge(zone->llfree) >>
				(suitable_order - HPAGE_PMD_ORDER);
	}
	return;
#endif
...
#ifdef CONFIG_LLFREE
	{
		unsigned long huge = zone->llfree ? llfree_free_huge(zone->llfree) : 0;
		unsigned long total = zone->llfree ? llfree_free_frames(zone->llfree) : 0;

		/* Aggregate bins, as in AE; detailed metadata is in /proc/llfree. */
		for (order = 0; order < NR_PAGE_ORDERS; order++)
			seq_printf(m, "%6lu ", order == HPAGE_PMD_ORDER ? huge :
				order == 0 ? total - (huge << HPAGE_PMD_ORDER) : 0UL);
		seq_putc(m, '\n');
		return;
	}
#endif
