...
unsigned long move_page_tables(struct pagetable_move_control *pmc)
{
	...
	/* A whole PMD/PUD move bypasses move_ptes(). Restore before any
	 * move (including DONTUNMAP and rollback), while mmap_write is held. */
	if (chameleon_shadow_restore_range(mm, pmc->old_addr, pmc->old_end))
		return 0;
