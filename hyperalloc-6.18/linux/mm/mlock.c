	       ...
	       unsigned long end, vm_flags_t newflags)
{
	...
	/* Shadow keeps resident rmap, so restoring it does not run the native
	 * rmap-add mlock hook. Restore before VMA changes so the ordinary mlock
	 * walk sees these resident pages, including MLOCK_ONFAULT requests. */
	ret = chameleon_shadow_restore_range(mm, start, end);
	if (ret)
		goto out;
