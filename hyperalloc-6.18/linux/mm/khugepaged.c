				       ...
				       int referenced)
{
	...
	for (addr = start_addr; addr < end; addr += PAGE_SIZE) {
		...
		if (!is_swap_pte(vmf.orig_pte))
			continue;
		/* A scan may have dropped mmap_lock before this recheck.
		 * Shadow is resident protocol state, never swap-in work. */
		if ((is_chameleon_shadow_entry(pte_to_swp_entry(vmf.orig_pte)) ||
		     is_chameleon_reclaimed_entry(pte_to_swp_entry(vmf.orig_pte)))) {
			pte_unmap(pte);
			mmap_read_unlock(mm);
			result = SCAN_PTE_NON_PRESENT;
			goto out;
...
struct chameleon_collapse {
	struct folio *source[HPAGE_PMD_NR];
	pte_t ptes[HPAGE_PMD_NR];
	unsigned long pfns[HPAGE_PMD_NR];
	unsigned int nr_sources;
};

#ifdef CONFIG_CHAMELEON_TEST
static atomic_t chameleon_collapse_failure;

void chameleon_collapse_fail_next(unsigned int phase)
{
	atomic_set(&chameleon_collapse_failure, phase);
}
#endif

/* Called with the original, temporarily detached PTE table locked. Every
 * source folio must be fully and exclusively mapped inside the target. */
static int chameleon_collapse_isolate(struct vm_area_struct *vma,
		unsigned long address, pte_t *ptes, unsigned int order,
		struct chameleon_collapse *cc)
{
	unsigned int source_order = order == 2 ? 0 : order - 1;
	unsigned int source_pages = 1U << source_order, nr = 1U << order;
	unsigned int i, j;

	for (i = 0; i < nr; i += source_pages) {
		pte_t entry = ptep_get(ptes + i);
		struct folio *folio;
		struct page *page;

		if (!pte_present(entry) || pte_protnone(entry) || pte_uffd_wp(entry))
			return -EINVAL;
		page = vm_normal_page(vma, address + i * PAGE_SIZE, entry);
		if (!page || is_zone_device_page(page))
			return -EINVAL;
		folio = page_folio(page);
		if (page != &folio->page || folio_order(folio) != source_order)
			return -EINVAL;
		if (!folio_trylock(folio))
			return -EBUSY;
		if (!folio_test_anon(folio) || folio_test_ksm(folio) ||
		    ...
		    folio_mapcount(folio) != source_pages ||
		    folio_ref_count(folio) != folio_expected_ref_count(folio)) {
			folio_unlock(folio);
			return -EBUSY;
		}
		for (j = 0; j < source_pages; j++) {
			unsigned long addr = address + (i + j) * PAGE_SIZE;

			entry = ptep_get(ptes + i + j);
			if (!pte_present(entry) || pte_protnone(entry) ||
			    pte_uffd_wp(entry) ||
			    vm_normal_page(vma, addr, entry) != folio_page(folio, j) ||
			    !PageAnonExclusive(folio_page(folio, j))) {
				folio_unlock(folio);
				return -EBUSY;
			}
			cc->ptes[i + j] = entry;
			cc->pfns[i + j] = pte_pfn(entry);
		}
		if (!folio_isolate_lru(folio)) {
			folio_unlock(folio);
			return -EBUSY;
		}
		node_stat_mod_folio(folio, NR_ISOLATED_ANON, source_pages);
		cc->source[cc->nr_sources++] = folio;
		/* Isolation owns one reference. Recheck after removing the LRU
		 * entry, closing the window for another LRU scanner's get. */
...
/* Internal caller retains mm_users, and holds no mmap/folio/PTL/LRU lock.
 * PMD collapse uses the native huge-PMD install/deposit primitives; smaller
 * targets keep the same PTE table and preserve every neighbouring PTE.
 * The strict source rules intentionally exclude holes, zero pages, swap,
 * shared/COW and externally referenced pages accepted by native khugepaged. */
			    ...
			    unsigned int order)
{
	...
	struct vm_area_struct *vma;
	struct folio *folio = NULL;
	unsigned long pmd_address = address & HPAGE_PMD_MASK;
	unsigned int nr, i, offset, failure = 0;
	spinlock_t *pmd_ptl, *pte_ptl;
	pmd_t *pmd, original_pmd, entry;
	pte_t *ptes;
	bool committed = false;
	int err = -EINVAL;

	if (order < 2 || order > HPAGE_PMD_ORDER ||
	    !IS_ALIGNED(address, PAGE_SIZE << order) ||
	    pmd_address > TASK_SIZE - HPAGE_PMD_SIZE)
		return -EINVAL;
	nr = 1U << order;
	...
	if (!cc)
		return -ENOMEM;
#ifdef CONFIG_CHAMELEON_TEST
	failure = atomic_xchg(&chameleon_collapse_failure, 0);
#endif
	if (failure == 1) {
		err = -ENOMEM;
		goto out;
	}
	folio = folio_alloc(GFP_TRANSHUGE, order);
	if (!folio) {
		err = -ENOMEM;
		goto out;
	}
	err = mem_cgroup_charge(folio, mm, GFP_TRANSHUGE);
	if (err)
		goto out;
	/* LRU batches retain temporary references; only stable sources qualify. */
	lru_add_drain_all();
	mmap_write_lock(mm);
	vma = find_vma(mm, address);
	err = -EINVAL;
	if (!vma || vma->vm_start > pmd_address ||
	    vma->vm_end < pmd_address + HPAGE_PMD_SIZE ||
	    !vma_is_anonymous(vma) || !vma->anon_vma ||
	    !(vma->vm_flags & VM_READ) ||
	    (vma->vm_flags & (VM_SHARED | VM_SPECIAL | VM_HUGETLB |
			      VM_LOCKED | VM_SHADOW_STACK | VM_DROPPABLE)) ||
	    userfaultfd_armed(vma))
		goto out_mmap;
	vma_start_write(vma);
	pmd = mm_find_pmd(mm, address);
	if (!pmd || check_pmd_state(pmd) != SCAN_SUCCEED)
		goto out_mmap;
	/* Neighbours are not isolated. Keep anon_vma exclusion until their
	 * original PTE table becomes reachable again, including all failures. */
	anon_vma_lock_write(vma->anon_vma);
	mmu_notifier_range_init(&range, MMU_NOTIFY_CLEAR, 0, mm, pmd_address,
				pmd_address + HPAGE_PMD_SIZE);
	mmu_notifier_invalidate_range_start(&range);
	pmd_ptl = pmd_lock(mm, pmd);
	original_pmd = pmdp_collapse_flush(vma, pmd_address, pmd);
	spin_unlock(pmd_ptl);
	tlb_remove_table_sync_one();

	ptes = pte_offset_map_lock(mm, &original_pmd, address, &pte_ptl);
	if (!ptes) {
		err = -EAGAIN;
		goto restore;
	}
	err = chameleon_collapse_isolate(vma, address, ptes, order, cc);
	pte_unmap_unlock(ptes, pte_ptl);
	/* pte_unmap also ends RCU read-side protection: copy and heat transfer
	 * below may sleep and must not retain that mapping across the calls. */
	if (err)
		goto restore;
	if (order == HPAGE_PMD_ORDER) {
		pgprot_t prot = pte_pgprot(pte_mkclean(pte_mkold(
					pte_clear_soft_dirty(cc->ptes[0]))));

		for (i = 1; i < nr; i++) {
			pgprot_t next = pte_pgprot(pte_mkclean(pte_mkold(
						pte_clear_soft_dirty(cc->ptes[i]))));

			if (pgprot_val(next) != pgprot_val(prot)) {
				err = -EINVAL;
				goto restore;
			}
		}
	...
	for (i = 0; i < nr; i++) {
		if (copy_mc_user_highpage(folio_page(folio, i),
					 pfn_to_page(cc->pfns[i]),
					 address + i * PAGE_SIZE, vma) ||
		    failure == 2) {
			err = -EIO;
			goto restore;
		}
	...
	err = chameleon_transfer_counters(folio_pfn(folio), cc->pfns, nr);
	if (err)
		goto restore;
	/* No operation below this point can fail. The new allocation reference
	 * becomes the mapping reference (one per PTE, or one for the huge PMD). */
	__folio_mark_uptodate(folio);
	folio_set_active(folio);
	/* CHAMELEON is x86-64 only: PTE tables use the permanent direct map.
	 * mmap_write + anon_vma_write and ownership of original_pmd keep this
	 * detached table alive. This accessor does not enter an RCU section. */
	BUILD_BUG_ON(IS_ENABLED(CONFIG_HIGHPTE));
	ptes = pte_offset_kernel(&original_pmd, address);
	pte_ptl = pte_lockptr(mm, &original_pmd);
	spin_lock(pte_ptl);
	offset = 0;
	for (i = 0; i < cc->nr_sources; i++) {
		struct folio *source = cc->source[i];
		unsigned int count = folio_nr_pages(source);

		clear_ptes(mm, address + offset * PAGE_SIZE, ptes + offset, count);
		folio_remove_rmap_ptes(source, &source->page, count, vma);
		folio_put_refs(source, count);
		offset += count;
	}
	if (order < HPAGE_PMD_ORDER) {
		...
		folio_add_lru_vma(folio, vma);
		for (i = 0; i < nr; i++)
			set_pte_at(mm, address + i * PAGE_SIZE, ptes + i,
				   pfn_pte(folio_pfn(folio) + i,
					   pte_pgprot(cc->ptes[i])));
	}
	spin_unlock(pte_ptl);
	spin_lock(pmd_ptl);
	if (order == HPAGE_PMD_ORDER) {
		entry = pmd_wrprotect(folio_mk_pmd(folio, vma->vm_page_prot));
		if (pte_write(cc->ptes[0]))
			entry = pmd_mkwrite(entry, vma);
		for (i = 0; i < nr; i++) {
			if (pte_young(cc->ptes[i]))
				entry = pmd_mkyoung(entry);
			if (pte_dirty(cc->ptes[i]))
				entry = pmd_mkdirty(entry);
			if (pte_soft_dirty(cc->ptes[i]))
				entry = pmd_mksoft_dirty(entry);
		}
		folio_add_new_anon_rmap(folio, vma, address, RMAP_EXCLUSIVE);
		folio_add_lru_vma(folio, vma);
		pgtable_trans_huge_deposit(mm, pmd, pmd_pgtable(original_pmd));
		set_pmd_at(mm, address, pmd, entry);
		update_mmu_cache_pmd(vma, address, pmd);
		deferred_split_folio(folio, false);
	} else {
		pmd_populate(mm, pmd, pmd_pgtable(original_pmd));
	}
	spin_unlock(pmd_ptl);
	folio = NULL;
	committed = true;
...
end_invalidate:
	...
	for (i = 0; i < cc->nr_sources; i++) {
		...
		node_stat_sub_folio(source, NR_ISOLATED_ANON);
		folio_unlock(source);
		if (committed) {
			/* Exclusive source has no mappings or references except
			 * isolation. Never put this now-dead folio back on the LRU. */
			folio_clear_active(source);
			folio_put(source);
		} else {
			folio_putback_lru(source);
		}
	}
out_mmap:
	mmap_write_unlock(mm);
				   ...
				   struct collapse_control *cc)
{
	...
	for (addr = start_addr, _pte = pte; _pte < pte + HPAGE_PMD_NR;
		...
		if (is_swap_pte(pteval)) {
			if ((is_chameleon_shadow_entry(pte_to_swp_entry(pteval)) ||
			     is_chameleon_reclaimed_entry(pte_to_swp_entry(pteval)))) {
				result = SCAN_PTE_NON_PRESENT;
				goto out_unmap;
			}
			++unmapped;
