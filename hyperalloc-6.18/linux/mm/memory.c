		...
		struct vm_area_struct *src_vma, unsigned long addr, int *rss)
{
	...
	/* copy_page_range() restores all Shadow entries before taking PTLs.
	 * Never duplicate a token/metadata association into the child mm. */
	if (WARN_ON_ONCE(is_chameleon_shadow_entry(entry) ||
			 is_chameleon_reclaimed_entry(entry)))
		return -EIO;
...
copy_page_range(struct vm_area_struct *dst_vma, struct vm_area_struct *src_vma)
{
	...
	/* dup_mmap holds mmap_write: restore before nested PTLs, notifier,
	 * and the write-protect seqcount. Native fork then owns normal COW. */
	ret = chameleon_shadow_restore_range(src_mm, addr, end);
	if (ret)
		return ret;
		...
		struct zap_details *details, int *rss, bool *any_skipped)
{
	...
	*any_skipped = true;
	...
	if (is_chameleon_reclaimed_entry(entry)) {
		/* Finalize already removed RSS/rmap/mapping references. The
		 * token is not a PFN and the physical reservation is independent. */
		if (!should_zap_cows(details))
			return 1;
		chameleon_reclaimed_zap(vma, addr, pte, ptent);
	} else if (is_chameleon_shadow_entry(entry)) {
		struct page *page = pfn_swap_entry_to_page(entry);
		struct folio *folio = page_folio(page);

		if (unlikely(!should_zap_folio(details, folio)))
			return 1;
		/* Retire metadata under this same PTL. The helper defers its
		 * final object/mm/isolation cleanup to a sleepable context. */
		chameleon_shadow_zap(vma, addr, pte, ptent);
		rss[MM_ANONPAGES]--;
...
vm_fault_t do_swap_page(struct vm_fault *vmf)
{
	...
	if (unlikely(non_swap_entry(entry))) {
		if (is_chameleon_reclaimed_entry(entry)) {
			/* A complete folio restore can cross adjacent VMAs. */
			if (vmf->flags & FAULT_FLAG_VMA_LOCK) {
				vma_end_read(vma);
				ret = VM_FAULT_RETRY;
			} else {
				ret = chameleon_reclaimed_fault(vmf);
			}
		} else if (is_chameleon_shadow_entry(entry)) {
			/* Restoring a folio can visit more than this one VMA. */
			if (vmf->flags & FAULT_FLAG_VMA_LOCK) {
				vma_end_read(vma);
				ret = VM_FAULT_RETRY;
			} else {
				ret = chameleon_shadow_fault(vmf);
			}
		} else if (is_migration_entry(entry)) {
