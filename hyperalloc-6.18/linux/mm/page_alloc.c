...
extern int ll_request_install(struct zone *, u64, unsigned int, size_t);
#endif
...
{
	chameleon_free_pages(pfn, order);
#ifdef CONFIG_LLFREE
	unsigned long flags;
	unsigned long base = ALIGN_DOWN(zone->zone_start_pfn, 1UL << LLFREE_MAX_ORDER);
	llfree_result_t result;
	unsigned int cpu;

	VM_BUG_ON(!zone->llfree);
	VM_BUG_ON_PAGE(bad_range(zone, page), page);
	cpu = get_cpu();
	local_irq_save(flags);
	result = llfree_put(zone->llfree, cpu, pfn - base, llflags(order));
	BUG_ON(!llfree_is_ok(result));
			...
			unsigned int order)
{
	...
	/* Clear before PCP visibility as well as on the LLFree direct path. */
	chameleon_free_pages(page_to_pfn(page), order);
			  ...
			  fpi_t fpi_flags)
{
	...
	if (IS_ENABLED(CONFIG_LLFREE)) {
		__free_one_page(page, pfn, zone, order,
			get_pfnblock_migratetype(page, pfn), fpi_flags);
...
#ifdef CONFIG_LLFREE
	if (zone->llfree) {
		llfree_result_t result = llfree_drain(zone->llfree, smp_processor_id());
		BUG_ON(!llfree_is_ok(result));
	}
	return;
#endif
...
#ifdef CONFIG_LLFREE
	if (zone->llfree) {
		llfree_result_t result = llfree_drain(zone->llfree, cpu);
		BUG_ON(!llfree_is_ok(result));
	}
	return;
#endif
...
static void __drain_all_pages(struct zone *zone, bool force_all_cpus)
{
	...
	/* LLFree reservations exist even though every PCP count is zero. */
	if (IS_ENABLED(CONFIG_LLFREE))
		force_all_cpus = true;
			...
			int migratetype)
{
	...
	llflags_t request = llflags(order);
	llfree_result_t result;

	request.movable = !!(gfp_flags & __GFP_MOVABLE);
	local_irq_save(flags);
	result = llfree_get(zone->llfree, cpu, request);
	page = NULL;
	if (llfree_is_ok(result)) {
		...
		if (result.reclaimed) {
#ifdef CONFIG_VIRTIO_LLFREE_BALLOON
			error = ll_request_install(zone, result.frame, order, cpu);
#else
			error = -ENODEV;
#endif
		}
		if (error) {
			llfree_result_t undone = llfree_put(zone->llfree, cpu,
							result.frame, llflags(order));
			BUG_ON(!llfree_is_ok(undone));
		} else {
			page = pfn_to_page(ALIGN_DOWN(zone->zone_start_pfn,
					1UL << LLFREE_MAX_ORDER) + result.frame);
			__mod_zone_page_state(zone, NR_FREE_PAGES, -(1 << order));
			__count_zid_vm_events(PGALLOC, zone_idx(zone), 1 << order);
			zone_statistics(preferred_zone, zone, 1 << order);
		}
	} else {
		BUG_ON(result.error != LLFREE_ERR_MEMORY);
	}
	local_irq_restore(flags);
	put_cpu();
	return page;
			 ...
			 long free_pages)
{
	...
	/* LLFree tracks contiguous availability in its shared bitmap. */
	if (IS_ENABLED(CONFIG_LLFREE))
		return true;
