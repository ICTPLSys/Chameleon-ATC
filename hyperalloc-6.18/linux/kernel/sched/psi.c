...
struct psi_memory_cpu_sample {
	u32 previous[3], delta[3];
};
static DEFINE_PER_CPU(struct psi_memory_cpu_sample, psi_memory_samples);
static DEFINE_MUTEX(psi_memory_sample_lock);
static u64 psi_memory_sample_stamp, psi_memory_sample_some, psi_memory_sample_full;

int psi_memory_snapshot(u64 *some_ns, u64 *full_ns, u64 *stamp_ns)
{
	const enum psi_states states[] = { PSI_MEM_SOME, PSI_MEM_FULL, PSI_NONIDLE };
	u64 now, nonidle = 0, some = 0, full = 0;
	bool baseline, expired;
	int cpu, i;

	if (static_branch_likely(&psi_disabled))
		return -EOPNOTSUPP;
	/* A private sampler must not consume PSI_AVGS/POLL times_prev: their
	 * jiffy-scaled weighting loses sub-jiffy samples. Read the same native
	 * scheduler buckets, including ongoing states, with independent cursors.
	 */
	mutex_lock(&psi_memory_sample_lock);
	now = ktime_get_ns();
	baseline = !psi_memory_sample_stamp;
	expired = !baseline && now - psi_memory_sample_stamp >= U32_MAX;
	for_each_possible_cpu(cpu) {
		struct psi_group_cpu *groupc = per_cpu_ptr(psi_system.pcpu, cpu);
		struct psi_memory_cpu_sample *sample = per_cpu_ptr(&psi_memory_samples, cpu);
		u64 clock, start;
		u32 mask, times[3];
		unsigned int seq;
		...
		do {
			seq = psi_read_begin(cpu);
			clock = cpu_clock(cpu);
			start = groupc->state_start;
			mask = groupc->state_mask;
			for (i = 0; i < 3; i++)
				times[i] = groupc->times[states[i]];
		} while (psi_read_retry(cpu, seq));
		for (i = 0; i < 3; i++) {
			if (mask & BIT(states[i]))
				times[i] += clock - start;
			sample->delta[i] = baseline || expired ? 0 : times[i] - sample->previous[i];
			sample->previous[i] = times[i];
		}
		nonidle += sample->delta[2];
	}
	if (nonidle) {
		/* Divide each weighted term with a full-width intermediate before
		 * summing: sum(stall_ns * nonidle_ns) could overflow on many CPUs.
		 * Rounding is below one nanosecond per contributing CPU/window.
		 */
		for_each_possible_cpu(cpu) {
			struct psi_memory_cpu_sample *sample = per_cpu_ptr(&psi_memory_samples, cpu);

			some += mul_u64_u64_div_u64(sample->delta[0], sample->delta[2], nonidle);
			full += mul_u64_u64_div_u64(sample->delta[1], sample->delta[2], nonidle);
		}
	}
	psi_memory_sample_some += some;
	psi_memory_sample_full += full;
	psi_memory_sample_stamp = now;
	*some_ns = psi_memory_sample_some;
	*full_ns = psi_memory_sample_full;
	*stamp_ns = now;
	mutex_unlock(&psi_memory_sample_lock);
	/* u32 native buckets may have wrapped more than once across a long
	 * suspended consumer. Rebaseline explicitly, never call that low PSI. */
	return expired ? -EAGAIN : 0;
}
EXPORT_SYMBOL_GPL(psi_memory_snapshot);
