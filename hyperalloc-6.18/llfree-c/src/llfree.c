			     ...
			     size_t order, reserved_t old)
{
	...
	if (!atom_cmp_exchange(&local->reserved[kind], &old, new)) {
		// Revert the steal
		// atom_update overwrites old_tree before evaluating its arguments.
		// Preserve the stolen amount across a racing update of this tree.
		treeF_t stolen_free = old_tree.free;
